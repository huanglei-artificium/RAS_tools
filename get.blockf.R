...
option_list <- list(
    ...
    make_option(c("--popDefFile"), type = "character", default = NA),
    make_option(c("--popLeft1"), type = "character"),
    make_option(c("--popLeft1Fillna"), type = "character", default = NA),
    make_option(c("--popLeft2"), type = "character", default = NA),
    make_option(c("--popLeft2Fillna"), type = "character", default = NA),
    make_option(c("--popRight1"), type = "character"),
    make_option(c("--popRight1Fillna"), type = "character", default = NA),
    make_option(c("--popRight2"), type = "character", default = NA),
    make_option(c("--popRight2Fillna"), type = "character", default = NA),
    make_option(c("--jackknife"), type = "character", default = NA),
    make_option(c("--ascertained"), action = "store_true", default = FALSE),
    make_option(c("--ascRefpop"), type = "character", default = NA),
    make_option(c("--ascFreqTable"), type = "character", default = NA),
    make_option(c("--allSitesBlock"), action = "store_true", default = FALSE),
    make_option(c("--refpopMaxMiss"), type = "double", default = NA),
    make_option(c("--refpopRemoveHomo"), action = "store_true", default = FALSE),
    make_option(c("--ascOutgroup"), type = "character", default = NA),
    make_option(c("--outgroupMaxHetero"), type = "double", default = NA),
    make_option(c("--outgroupRoundFreq"), action = "store_true", default = FALSE),
...
pop_def_file <- opt$popDefFile

pop_left1_str <- opt$popLeft1

pop_left1_fillna <- opt$popLeft1Fillna

pop_left2_str <- opt$popLeft2

pop_left2_fillna <- opt$popLeft2Fillna

pop_right1_str <- opt$popRight1

pop_right1_fillna <- opt$popRight1Fillna

pop_right2_str <- opt$popRight2

pop_right2_fillna <- opt$popRight2Fillna

jackknife <- opt$jackknife

is_ascertained <- opt$ascertained

asc_refpop <- opt$ascRefpop

asc_freq_table <- opt$ascFreqTable

all_sites_block <- opt$allSitesBlock

refpop_max_miss <- opt$refpopMaxMiss %>% as.numeric()

refpop_remove_homo <- opt$refpopRemoveHomo

asc_outgroup <- opt$ascOutgroup

outgroup_max_hetero <- opt$outgroupMaxHetero %>% as.numeric()

outgroup_round_freq <- opt$outgroupRoundFreq

out_file <- opt$outFile

##################

base::source("/mnt/archgen/users/lei_huang/RAS_tools/RAS_src.R")
...
get_freq <- function(pop, lth, columns, keep_frac = FALSE)
{
    #
    ...
    } else    # pop %in% names(pop_def)
    {
        ...
        {
            get_freq(pop0, lth, columns, keep_frac = TRUE)
...
message("pop_left1:")
pop_left1 <- pop_str_convert(pop_left1_str, names(pop_info))
message("pop_right1:")
pop_right1 <- pop_str_convert(pop_right1_str, names(pop_info))
...
get_freq(asc_outgroup, lth = "full", column_outgroup, keep_frac = FALSE)
rm(column_outgroup)
# We define a vector to indicate whether a specific site is kept (TRUE) or not (FALSE), with the length equal to the genome length of outgroup. In this step, we only consider "outgroup_max_hetero".
filter_all <- if (is.na(outgroup_max_hetero)) !is.na(pop_info[[asc_outgroup]]$freq) else pop_info[[asc_outgroup]]$freq <= outgroup_max_hetero | pop_info[[asc_outgroup]]$freq >= 1 - outgroup_max_hetero
...
if (is_ascertained)
{
    ...
    get_freq(asc_refpop, lth = "full", column_refpop, keep_frac = TRUE)
    rm(column_refpop)
...
lapply(pop_all, get_freq, lth = "asc", columns = column_test_related, keep_frac = FALSE)
...
get_blockfsum <- function(asc_outgroup, pop_left1, pop_right1)
{
    ...
    if (!is.na(asc_freq_table))
    {
        ...
        for (i in 1:nrow(asc_conds))
        {
            ...
            f_value_block_table <- f_value_block_table %>% rbind(data.table::data.table(min_freq = asc_conds$min_freq[i], max_freq = asc_conds$max_freq[i], Nr_sites_all = fsum_block$Nr_sites_all, Nr_sites_nonmissing = fsum_block$Nr_sites_nonmissing, f = fsum_block$fsum / fsum_block$Nr_sites_nonmissing, block_id = block_id_unique))
    ...
    } else
    {
        ...
        f_value_block_table <- data.table::data.table(Nr_sites_all = fsum_block$Nr_sites_all, Nr_sites_nonmissing = fsum_block$Nr_sites_nonmissing, f = fsum_block$fsum / fsum_block$Nr_sites_nonmissing, block_id = block_id_unique)
...
if (!is.na(out_file))
{
    write.table(f_block_table_all, out_file, quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
} else
    print(f_block_table_all)
