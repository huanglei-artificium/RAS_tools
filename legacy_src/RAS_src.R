...
get_freq <- function(pops, lth, mask, full_info)
{
    ...
    for (file in file_all)
    {
        ...
        columns <- if (is_geno) readr::read_fwf(file, col_positions=readr::fwf_positions(field, field, col_names=column_name), col_types=readr::cols(.default="i"), na="9") else data.table::fread(file=file, header=FALSE, select=field, na.strings=c("-1", "*"), col.names=column_name)
        if (!is.null(mask))
        ...
        for (pop in pop_name_uniq)
        {
            ...
            } else # frac1 || freq, do not distinguish total>1 or total == 1 now. Problem: what if haploid calling with total 1? its not frequency and can be added!
            {
...
{
    asc_outgroup_freq <- full_info$freq[[asc_outgroup]]
    pop_left1_freq <- full_info$freq[[pop_left1]] %>% {if (!is.na(full_info$pop_left_fillna)) fill_na_with_ref(., ref=if (full_info$pop_left_fillna=="Outgroup") full_info$outgroup_freq else 0) else .}
    if (!is.null(pop_left2))
    {
        pop_left2_freq <- full_info$freq[[pop_left2]] %>% {if (!is.na(full_info$pop_left_fillna)) fill_na_with_ref(., ref=if (full_info$pop_left_fillna=="Outgroup") full_info$outgroup_freq else 0) else .}
        pop_left_freq <- pop_left2_freq - pop_left1_freq
    } else {
       pop_left_freq <- asc_outgroup_freq - pop_left1_freq
    }
    pop_right1_freq <- full_info$freq[[pop_right1]] %>% {if (!is.na(full_info$pop_right_fillna)) fill_na_with_ref(., ref=if (full_info$pop_right_fillna=="Outgroup") full_info$outgroup_freq else 0) else .}
    if (!is.null(pop_right2))
    {
        pop_right2_freq <- full_info$freq[[pop_right2]] %>% {if (!is.na(full_info$pop_right_fillna)) fill_na_with_ref(., ref=if (full_info$pop_right_fillna=="Outgroup") full_info$outgroup_freq else 0) else .}
