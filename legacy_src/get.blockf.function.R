...
# full_info$columns$posi$frequency_file[1]
if (is.na(position_file))
{
    position <- data.table::fread(file=full_info$columns$posi$frequency_file[1], header=FALSE, select=c(1,2), na.strings=c("-1", "*"), col.names=c("CHR", "POS"))[filter_all]
    # pop_to_freq(c("CHR", "POS"), full_info$columns$posi, mask = filter_all)
} else
{
    position_all <- data.table::fread(file=full_info$columns$posi$frequency_file[1], header=FALSE, select=c(1,2), na.strings=c("-1", "*"), col.names=c("CHR", "POS"))
    # pop_to_freq(c("CHR", "POS"), full_info$columns$posi)
...
if (is_ascertained)
{
   ...
   full_info$RASD_table_all <- purrr::pmap(full_info$all_stats_RASD, get_blockf_nonasc, full_info = full_info)
}


f_block_table_all <- base::mapply(base::cbind, full_info$RASD_table_all, full_info$all_stats_RASD %>% base::split(seq_len(nrow(.))), SIMPLIFY = FALSE) %>% dplyr::bind_rows()
